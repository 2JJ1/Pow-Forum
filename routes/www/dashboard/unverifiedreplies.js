...
router.get('/', async (req, res, next) => {
    try {
        ...
        let replies = await ThreadReplies.find({verified: false}).sort({_id: -1}).skip((page-1)*25).limit(25).lean()
        ...
        res.render('pages/dashboard/unverifiedreplies', {...pagedata, replies})
    }
    catch(e){
        next(e)
    }
