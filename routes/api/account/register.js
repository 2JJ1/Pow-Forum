...
router.post('/', async (req, res) => {
	...
	try {
		...
		.then(async doc => {
			...
			mailgun.SendBasicEmail(keyvalues.email, `${(await ForumSettings.findOne({type: "name"})).value} | Email Verification`, emailBody)
			.catch(err=>{
		...
		if(typeof error === 'object' && 'safe' in error) 
			response.reason = error.safe
		else{
			console.warn(error)
